...
public class DebugLog { 
	...
	/**
	 * By default (<code>false</code>) log messages are kept until explicitly written. 
	 * However, if this is <code>true</code>, they are written to <code>stdout</code> immediately. 
	 */
	public static boolean shouldAlwaysFlush = false ; 
	
	public static int remainingMessageCount = -1 ; 
	...
	public static void addPlain(double requiredVerbosity, Block block, Canvas canvas, String message) { 
		...
		if ((block!=null&&requiredVerbosity<=block.loggingVerbosity) || (canvas!=null&&requiredVerbosity<=canvas.loggingVerbosity) || (requiredVerbosity<=loggingVerbosity)) { 
			synchronized (lock) {
				...
				if (shouldAlwaysFlush) out(); 
				if (remainingMessageCount>=0 && remainingMessageCount--<=0) throw new RuntimeException("Log message limit reached."); 
			}
		}
	}
	 ...
	 */
	public static void add(double requiredVerbosity, Block block, String message, boolean wantStack) { 
		if (requiredVerbosity<=0) return ; 
		if ((block!=null&&requiredVerbosity<=block.loggingVerbosity) || (requiredVerbosity<=loggingVerbosity)) { 
