...
public class FrameHorizontal extends Frame {
		...
		public PlacedBlock fill(Quill receivedQuill, Layout receivedLayout) throws IOException { 
			...
			if (frameLayout.isWidthTight) { 
				...
				gap = getHorizontalGap(quill) + ((justification==Layout.FULL && childCount>0) ? spareSpace / (childCount-1) : 0) ; // Horizontal gap between words. 
				if(justification==Layout.FULL) { 
					nextLeft = 0 ; 
				} else {
					nextLeft = Layout.getJustificationSpace(justification, spareSpace);
				}
			}
