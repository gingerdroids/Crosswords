...
public abstract class TableBlock extends Block implements DebugLog.Verbosity { 
	...
	/**
	 * Table cell background {@linkplain Block} instances, first index is row, second is column. 
	 */
	public final Block[][] blockBackgroundRows ; 
	
	/**
	 * Table cell background {@linkplain PlacedBlock} instances, first index is row, second is column. 
	 */
	public final PlacedBlock[][] placedBackgroundBlockRows ; 
	...
	protected TableBlock(int rowCount, int columnCount) { 
		...
		this.blockBackgroundRows = new Block[rowCount][] ; 
		this.placedBackgroundBlockRows = new PlacedBlock[rowCount][] ; 
		for (int row=0 ; row<rowCount ; row++) blockBackgroundRows[row] = new Block[columnCount] ; 
		for (int row=0 ; row<rowCount ; row++) placedBackgroundBlockRows[row] = new PlacedBlock[columnCount] ; 
	...
	protected void fillCellArrays() { 
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
				Block cellBackgroundBlock = getBackgroundBlock(row, column); 
				blockBackgroundRows[row][column] = cellBackgroundBlock ; 
	...
	/**
	 * Returns the {@link Block} used to draw the background of the given cell. 
	 * It should fill the interior (including the padding!) of the cell, though the world doesn't end if it doesn't. 
	 * <p>
	 * A cell background is drawn before the borders and cell content. 
	 * <p>
	 * The size of the background block is computed after the size of the content block. 
	 */
	protected Block getBackgroundBlock(int row, int column) { 
		return null ; 
	}
	...
	/**
	 * Padding between the border of the cell and the content inside it. 
	 * @param row
	 * @param column
	 * @param isSide
	 * @param isBefore
	...
	public PlacedBlock fill(Quill receivedQuill, Layout receivedLayout) throws IOException { 
		...
		Layout cellBackgroundLayout = tableLayout.copy().setTight(false, false); // Layout object passed into cell backgrounds. Might be modified by calls to inheritCellBackgroundLayout().
		DebugLog.add(DETAIL_8, this, null, Layout.logMessage_layout, tableLayout, null, false); 
		//////  Invoke a 'fill' pass on each cell. 
		fillCellArrays(); 
		//////  Measure, and internally layout, each cell's content 
		/* 
		 * Cell content blocks drive the sizing of the cells. 
		 * The background blocks are sized later. 
		 */
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
		...
		double rowTop = 0 ; 
		for (int row=0 ; row<rowCount ; row++) { 
			rowTop += row==0 ? getTableBorderThickness(false, true) : getCellBorderThickness(row, false); 
			double rowHeight = 0 ; 
			for (int column=0 ; column<columnCount ; column++) { 
				PlacedBlock placedBlock = placedBlockRows[row][column];
				if (placedBlock==null) continue ; 
				double cellHeight = placedBlock.getHeight() + getCellPadding(row, column, false, true) + getCellPadding(row, column, false, false); 
		...
		double tableHeight = rowBottoms[rowCount-1] + getTableBorderThickness(false, false); 
		//////  Compute column widths, lefts, and rights. 
		double columnLeft = 0 ; 
		for (int column=0 ; column<columnCount ; column++) { 
			columnLeft += column==0 ? getTableBorderThickness(true, true) : getCellBorderThickness(column, true); 
			double columnWidth = 0 ; 
			for (int row=0 ; row<rowCount ; row++) { 
				PlacedBlock placedBlock = placedBlockRows[row][column];
				if (placedBlock==null) continue ; 
				double cellWidth = placedBlock.getWidth() + getCellPadding(row, column, true, true) + getCellPadding(row, column, true, false); 
		...
		double tableWidth = columnRights[columnCount-1] + getTableBorderThickness(true, false); 
		//////  Measure, and internally layout, each cell's background 
		/* 
		 * Background blocks do not control their size. They derive  their size from the cell. 
		 * In turn, the cell's derive their size from the cell content blocks. 
		 */
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
				Block cellBackgroundBlock = blockBackgroundRows[row][column]; 
				if (cellBackgroundBlock==null) continue ; 
				PlacedBlock cellBackgroundPlacedBlock = cellBackgroundBlock.fill(quill, inheritCellBackgroundLayout(cellLayout, tableLayout, this, row, column)); 
				placedBackgroundBlockRows[row][column] = cellBackgroundPlacedBlock ; 
			}
		}
		...
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
				PlacedBlock placedBlock = placedBlockRows[row][column];
				if (placedBlock==null) continue ; 
				placedBlock.setOffsetInContainer(columnLefts[column], rowTops[row]); 
			}
		}
		//////  Set my dimensions
	 ...
	 */
	protected Layout inheritCellLayout(Layout tableCellLayout, Layout tableLayout, TableBlock tableBlock, int row, int column) { 
	 ...
	 * Computes, or passes through, the {@linkplain} Layout} object to be used in the given cell. 
	 * <p>
	 * This implementation creates a copy, and sets the size to be the full size of the cell. 
	 * <p>
	 * This implementation does a raster scan through the cells: On the top row from left to right, then each row downwards.  
	 */
	protected Layout inheritCellBackgroundLayout(Layout tableCellBackgroundLayout, Layout tableLayout, TableBlock tableBlock, int row, int column) { 
		return tableCellBackgroundLayout.copy().setSize(columnRights[column]-columnLefts[column], rowBottoms[row]-rowTops[row]) ; 
	}
	
	@Override
	public void draw(Canvas canvas, double left, double top, double width, double height) throws IOException { 
		//////  Draw cell backgrounds (before borders). 
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
				PlacedBlock cellBackgroundPlacedBlock = placedBackgroundBlockRows[row][column]; 
				if (cellBackgroundPlacedBlock==null) continue ; 
				//// Compute boundaries of background - ignores padding, touches borders. 
				double cellInteriorLeft = columnLefts[column] ; 
				double cellInteriorRight = columnRights[column] ; 
				double cellInteriorTop = rowTops[row] ;
				double cellInteriorBottom = rowBottoms[row] ;
				...
				double contentLeftOffset = cellInteriorLeft + computeJustificationOffset(horizontalJustification, cellInteriorRight-cellInteriorLeft, cellBackgroundPlacedBlock.getWidth()); 
				double contentTopOffset = cellInteriorTop + computeAlignmentOffset(verticalAlignment, cellInteriorBottom-cellInteriorTop, cellBackgroundPlacedBlock.getHeight()); 
				//// Draw the content
				cellBackgroundPlacedBlock.draw(canvas, left+contentLeftOffset, top+contentTopOffset); 
			}
		}
		...
		for (int row=0 ; row<rowCount ; row++) { 
			for (int column=0 ; column<columnCount ; column++) { 
				...
				if (cellPlacedBlock==null) continue ; 
				//// Compute boundaries of content - ie, inside the padding. 
				double cellContentLeft = columnLefts[column] + getCellPadding(row, column, true, true); // Left boundary of content of cell (ie, inside the padding). 
				double cellContentRight = columnRights[column] - getCellPadding(row, column, true, false); 
				double cellContentTop = rowTops[row] + getCellPadding(row, column, false, true);
				double cellContentBottom = rowBottoms[row] - getCellPadding(row, column, false, false);
				//// Compute offsets to implement the required justification (cell content may be smaller than the cell interior)
				Justification horizontalJustification = getHorizontalJustification(row, column); 
				Alignment verticalAlignment = getVerticalAlignment(row, column); 
				double contentLeftOffset = cellContentLeft + computeJustificationOffset(horizontalJustification, cellContentRight-cellContentLeft, cellPlacedBlock.getWidth()); 
				double contentTopOffset = cellContentTop + computeAlignmentOffset(verticalAlignment, cellContentBottom-cellContentTop, cellPlacedBlock.getHeight()); 
				//// Draw the content
